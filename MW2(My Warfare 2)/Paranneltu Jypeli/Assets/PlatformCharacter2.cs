...
public class PlatformCharacter2 : PhysicsObject
{
    ...
    {
        PhysicsGame physGame = Game.Instance as PhysicsGame;
    ...
    private void AddCollisionHandler()
    {
        ...
        this.Body.Colliding += delegate( object sender, CollisionEventArgs args )
        {
            PhysicsObject other = (PhysicsObject)args.Other.Tag;
            Vector normal = new Vector( args.Contact.Points[0].Normal.X, args.Contact.Points[0].Normal.Y );
    ...
    protected override void PrepareThrowable( PhysicsObject obj, Angle angle, double force, double distanceDelta )
    {
        double d = ( this.Width + obj.Width ) / 2;
        Angle throwAngle = FacingDirection == Direction.Left ? Angle.StraightAngle - angle : angle;
        obj.Position = this.AbsolutePosition + this.FacingDirection.GetVector() * d;
