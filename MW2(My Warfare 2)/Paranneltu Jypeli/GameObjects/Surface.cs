...
namespace Jypeli
{
    ...
    public class Surface : PhysicsObject
    {
        ...
        public Surface( double width, double[] heights, double scale )
            : this( width, heights, scale, CalculateShapeParams( width, heights ) )
        {
        }

        public Surface( double width, double[] heights, double scale, CollisionShapeParameters shapeParameters )
            : base( width, CalculateHeight( heights, scale ), CreateRuggedShape( width, heights, scale ), shapeParameters )
        {
        ...
        public Surface( double width, double height, CollisionShapeParameters shapeParameters )
            : base( width, height, Shape.Rectangle, shapeParameters )
        {
            InitializeFlat( height );
        }
        ...
        public Surface( double width, double min, double max, int points, CollisionShapeParameters shapeParameters )
            : this( width, RandomGen.NextDoubleArray( Math.Max( min, 1.0f ), max, points ), 1.0, shapeParameters )
        {
        }
        ...
        public Surface( double width, double min, double max, int points, int maxchange, CollisionShapeParameters shapeParameters )
            : this( width, RandomGen.NextDoubleArray( Math.Max(min, 1.0f), max, points, maxchange ), 1.0, shapeParameters )
        {
        }
        ...
        private static CollisionShapeParameters CalculateShapeParams( double width, double[] heights )
        {
            int n = heights.Length;
            double step = width / ( n - 1 );

            CollisionShapeParameters parms;
            parms.DistanceGridSpacing = Math.Max( step / 8, GetMinHeightDifference( heights ) / 2 );
            parms.MaxVertexDistance = 20;
            return parms;
        }
